...
if ( isset($_POST['submit']) ) {
	...
	if ( empty($_POST['caption']) && empty($_POST['title']) && empty($_POST['date']) ) {
		$message = "Please select a field for the query.";
	} else {
		// caption
		if ( isset($_POST['caption']) ) {
			$query  = "SELECT * ";
			$query .= "FROM images ";
			$query .= "WHERE image_caption LIKE '%$search%'";
			$image_set = $mysqli -> query($query);
		
			if($image_set -> num_rows == 0) {
		...
		if ( isset($_POST['date']) ) {
			$query  = "SELECT * ";
			$query .= "FROM images ";
			$query .= "WHERE image_date_taken LIKE '%$search%'";
			$image_set = $mysqli -> query($query);
...
<div class="col-12 text-center">
	...
	<div class="container-small">
		<form action="#" method="POST">
			<div class="col-9">
				<input type="text" name="search" value="" size="30" maxlength="30" placeholder="Enter a search query">
			</div>
			<div class="col-3">
				<input class="btn-primary" type="submit" name="submit" value="Search">
			</div>
			<label class="col-12 label-align-horizontal">
			<span>Select the fields for the query</span><br>
				<input type="checkbox" class="align-left" name="caption" value="caption"> 
				<span>Image Caption</span>
				<input type="checkbox" class="align-left" name="date" value="date"> 
				<span>Image Date Taken</span>
			</label>
		</form>
		...
		<div class="col-12 border-bottom">
			...
			<div class="col-6 text-right">
				<form action="delete_image.php" method="POST">
					...
					<input type="hidden" name="image_caption" value="<?php echo $image["image_caption"]; ?>">
					<input type="hidden" name="image_date_taken" value="<?php echo $image["image_date_taken"]; ?>">
				</form>
			</div>

			<!-- EDIT button -->
			<div class="col-6 text-left">
			   <form action="admin.php#<?php echo $image["image_id"] ?>" method="POST">
...
<div class="container">	
	<ul class="col-12 text-center">
		<li><a class="nav go-top" href="<?php echo($site_root); ?>/php/admin.php">Back to Top</a></li>
	</ul>
</div>
